...
class VIDDataset:
	...
	def __init__(self, root, transform=None, target_transform=None, is_val=False,is_test=False, keep_difficult=False, label_file=None):
		...
		self.is_val = is_val
		if is_test:
			image_sets_file = "test_VID_seqs_list.txt"
		elif is_val:
			image_sets_file = "val_VID_seqs_list.txt"
		else:
			image_sets_file = "train_VID_seqs_list.txt"
		self.seq_list = VOCDataset._read_image_seq_ids(image_sets_file)
		...
		self._name_to_class = {self._classes_map[i]: self._classes_name[i] for i in range(len(self.class_names))}
		self._class_to_ind = {class_name: i for i, class_name in enumerate(self.class_names)}

	def __getitem__(self, index):
		image_seq = self.seq_list[index]
		boxes_seq, labels_seq, is_difficult_seq = self._get_annotation(image_seq)
		if not self.keep_difficult:
			for i in range(0,len(boxes_seq)):
				boxes_seq[i] = boxes_seq[i][is_difficult_seq[i] == 0]
				labels_seq[i] = labels_seq[i][is_difficult_seq[i] == 0]
		images = self._read_image(image_seq)
		if self.transform:
			for i in range(0,len(boxes_seq)):
				images[i], boxes_seq[i], labels_seq[i] = self.transform(images[i], boxes_seq[i], labels_seq[i])
		if self.target_transform:
			for i in range(0,len(boxes_seq)):
				boxes_seq[i], labels_seq[i] = self.target_transform(boxes_seq[i], labels_seq[i])
	...
	def _get_annotation(self, image_seq):
		...
		for annotation_file in annotation_files:
			...
			is_difficult_seq.append(np.array(is_difficult, dtype=np.uint8))
		return (boxes_seq, labels_seq, is_difficult_seq)
	...
	def _read_image(self, image_seq):
		if self.is_test:
			...
			images_seq = []
			for image_id in image_ids:
				image_file = self.root / f"Data/VID/val/{image_path}{image_id}"
				image = cv2.imread(str(image_file))
				image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
			...
			images_seq = []
			for image_id in image_ids:
				image_file = self.root / f"Data/VID/train/{image_path}{image_id}"
				image = cv2.imread(str(image_file))
				image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
				images_seq.append(image)
			return images_seq
